...
export function useBackendData() {
  ...
  const updateFoodLogTime = useCallback(async (id: string, newTime: string) => {
    // Update locally immediately for a snappy UI
    setFoodLogs(prev =>
      prev.map(log => log.id === id ? { ...log, timestamp: newTime } : log)
    );
    // Then try to persist to backend
    if (backendConnected) {
      try {
        await foodLogAPI.updateTime(id, newTime);
      } catch {
        // Backend failed — local update already happened so no need to revert
      }
    }
  }, [backendConnected, setFoodLogs]);
